...
<section id="tldr">
  <div class="wrap narrow">
    ...
    <details class="abstract">
      ...
      <div>
      <p>Learning-based co-design returns a <em>ranking</em> of candidate bodies scored in an abstract simulator whose cheapness makes the search tractable. We ask how much of that ranking survives the step that makes a design buildable. Using 18 morphologies of a modular icosahedral legged robot — a stratified sample of our own evolutionary co-design space — we train each with a shared PPO recipe in a replica of the abstract space the search used and in a fabricable space calibrated against CAD and motor datasheets. The two rankings are uncorrelated (ρ<sub>s</sub> = +0.11, p = 0.66, mean |Δrank| = 5.9 of 18): the abstract winner falls to 7th, and a design paralyzed in the abstract space (0.54 m, 17th) comes 2nd at 10.8 m. The reshuffle survives a 6×3 seed replication (P &lt; 0.01 under resampling), reward re-weighting and mass loading within the abstract space (ρ<sub>s</sub> ≥ 0.77), and halving the training budget (ρ<sub>s</sub> ≥ 0.90); a servo-level split attributes it to the geometry–DoF–rest-pose–mass half of realization rather than the actuator model (ρ<sub>s</sub> = −0.30 vs +0.61). At fixed realization, changing the <em>task environment</em> preserves the ranking (rubble ρ<sub>s</sub> = 0.90, ice 0.72); only discrete footholds are as disruptive. We trace the reshuffle to checkable mechanisms — a shell-sliding exploit, penetrating or floating rest poses, a leg-count advantage that exists only on paper — and argue that fidelity belongs inside the co-design loop, not after it.</p>
      </div>
    </details>
  </div>
  <div class="wrap">
    <div class="numbers">
      ...
      <div><div class="v">P &lt; 0.01</div><div class="k">reshuffle vs. seed noise (6 × 3 replication)</div></div>
    </div>
  </div>
</section>
